 ...
 * =============
 *
 * Queries are normally defined in advance.  This allows Caqti to emit prepared
 * queries which are reused throughout the lifetime of the connection. *)
...
(* Db.exec runs a statement which must not return any rows.  Errors are reported
 * as exceptions. *)
...
(* There is also a Db.iter_p for parallel processing, and Db.fold and Db.fold_s
 * for accumulating information from the result rows. *)
...
let test stdout db =
  ...
  let show_owner frameno =
    find_bike_owner frameno db |> Result.map @@ function
    | Some owner ->
        Flow.copy_string
          (Printf.sprintf "%s is owned by %s.\n" frameno owner) stdout
    | None ->
        Flow.copy_string
          (Printf.sprintf "%s is not registered.\n" frameno) stdout
  in
...
let () = Eio_main.run @@ fun env ->
  let url =
    Printf.sprintf "postgresql://%s%s%s%s"
      ...
      (match Sys.getenv_opt "PGDATABASE" with Some db -> "/"^db | None -> "")
    |> Uri.of_string
  in
  (* Note: Caqti_eio_unix is required for the postgresql driver, while the pure
   * ocaml pgx can be used with Caqti_eio*)
  Caqti_eio_unix.with_connection
    ~stdenv:(env :> Caqti_eio.stdenv) url (test env#stdout)
  |> Caqti_eio.or_fail
